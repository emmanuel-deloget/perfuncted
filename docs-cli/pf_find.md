## pf find

Pixel scanning and wait utilities

### Options

```
  -h, --help   help for find
```

### Options inherited from parent commands

```
      --max-x int32   input coordinate space width (default 1920)
      --max-y int32   input coordinate space height (default 1080)
      --nested        auto-detect and connect to a nested Wayland session in /tmp
```

### SEE ALSO

* [pf](pf.md)	 - perfuncted — screen automation CLI
* [pf find color](pf_find_color.md)	 - Find the first pixel matching a colour within tolerance
* [pf find last-pixel](pf_find_last-pixel.md)	 - Print the RGB colour of the bottom-right pixel of a region
* [pf find locate](pf_find_locate.md)	 - Find a reference PNG image within a screen region
* [pf find pixel-hash](pf_find_pixel-hash.md)	 - Print the CRC32 pixel hash of a screen region
* [pf find scan-for](pf_find_scan-for.md)	 - Scan multiple regions until one matches its expected hash
* [pf find wait-for](pf_find_wait-for.md)	 - Wait until a region's pixel hash equals the provided hash
* [pf find wait-for-change](pf_find_wait-for-change.md)	 - Wait until a region's pixel hash changes from an initial value
* [pf find wait-for-no-change](pf_find_wait-for-no-change.md)	 - Wait until a region's pixel hash is stable for N consecutive samples
* [pf find wait-locate](pf_find_wait-locate.md)	 - Poll until a reference image is found in the search area

###### Auto generated by spf13/cobra on 13-Apr-2026
